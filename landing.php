...
<html>
    ...
    <body>
        ...
        <form class="ajout" method='post' action='fil_traitement.php'>
            <input name="message_poster" class="input_ajout"type="textarea" placeholder="Publier un post" required/>
            <button id="button_ajouter" type="submit">Poster</button>
        </form>
        <?php 
        $reponse = $bdd->query('SELECT pseudo_auteur, contenue FROM messages ORDER BY ID DESC LIMIT 0, 10');
        // Affichage de chaque message (toutes les données sont protégées par htmlspecialchars)
        while ($data = $reponse->fetch())
        {
        ?>
        <div class="poste">
          <p class="pp"><img src="images/ppvide.jpg" alt="photo de profil" id="pp"></p>
            <div class="texte_post">
                <div class="point">
                    <p class="pseudopost"><?php echo $data['pseudo_auteur'] ?></p>
                    <p class="option">...</p>
                </div>
                <p class="texte"><?php echo $data['contenue'] ?></p>
            </div>
        </div>
        <?php
        }
        ...
        ?>
        <div class="plus">
            <button id="button_plus" type="submit">Plus de post +</button>
        </div>  
    </body>
</html>