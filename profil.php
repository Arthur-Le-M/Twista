...
<html>
    ...
    <body>
        ...
        //On récupère les info de l'utilisateur grâce au pseudo rentré dans la barre de recherche
        $recup_info = $bdd->prepare('SELECT pseudo, date_inscription FROM utilisateurs WHERE pseudo = ?');
        $recup_info->execute(array($pseudo));
        $data = $recup_info->fetch();
        $row = $recup_info->rowCount();
        if($row > 0){
            ?>
            
            <p class="pseudo_profil"><?php echo $pseudo ?></p>
            <?php
        }
        else{
            ?>
            <p class="erreur"> profil inexistant </p>
            <?php
        }
        ?>
        <div class="menu">
